...
@section('page-title', 'Edit Data Kurir')
        ...
        <div class="bg-gradient-to-r from-tni-800 to-tni-700 px-8 py-6 text-white">
            <div class="flex items-center">
                <div class="w-12 h-12 bg-white/20 rounded-xl flex items-center justify-center mr-4 backdrop-blur-sm border border-white/30">
                    <i class="fas fa-user-edit text-2xl text-gold-400"></i>
                </div>
                <div>
                    <h3 class="text-xl font-bold">Edit Data: {{ $courier->name }}</h3>
                    <p class="text-tni-100 text-sm">Perbarui informasi personel kurir</p>
                </div>
            </div>
        </div>

        <form action="{{ route('couriers.update', $courier) }}" method="POST" class="p-8 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $courier->name) }}" required
                ...
                <div class="space-y-2">
                    ...
                    <input type="email" name="email" id="email" value="{{ old('email', $courier->email) }}" required
                        class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-tni-500 focus:border-tni-500 transition-all outline-none"
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="phone" id="phone" value="{{ old('phone', $courier->phone) }}"
                        class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-tni-500 focus:border-tni-500 transition-all outline-none"
                ...
                <!-- Password Info -->
                <div class="col-span-full bg-blue-50 p-4 rounded-xl border border-blue-100 flex items-start">
                ...
                <div class="space-y-2">
                    <label for="password" class="text-sm font-bold text-gray-700 flex items-center">
                        <i class="fas fa-lock mr-2 text-tni-500"></i> Password Baru
                    </label>
                ...
                <div class="space-y-2">
                    <label for="password_confirmation" class="text-sm font-bold text-gray-700 flex items-center">
                        <i class="fas fa-shield-alt mr-2 text-tni-500"></i> Konfirmasi Password Baru
                    </label>
